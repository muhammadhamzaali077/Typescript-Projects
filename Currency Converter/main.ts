...
let apiLink =
  "https://v6.exchangerate-api.com/v6/9d4613ccf6c58bc3aed47e15/latest/PKR";
...
let userMoney = await inquirer.prompt({
  type: "number",
  name: "money",
  message: `Please enter the amount in ${chalk.bold.greenBright(
    firstCountry.name
  )}:`,
});
...
console.log(
  `Your ${chalk.bold.blueBright(firstCountry.name)} ${chalk.bold.blueBright(
    userMoney.money
  )} in ${chalk.bold.blueBright(secondCountry.name)} is ${chalk.bold.blueBright(
    convertedRates
  )}:`
);
